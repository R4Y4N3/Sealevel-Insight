...
export class InsightExplorer implements vscode.TreeDataProvider<Item> {
  ...
  getChildren(element?: Item): Item[] {
    ...
    if (element.kind === 'instruction-state-flows') return (program.stateFlows ?? []).filter(item => element.ids?.includes(item.id)).map(item => sourceOrText(`${program.accounts.find(account => account.id === item.accountId)?.name ?? item.accountId} • ${item.operations.join(', ')}${item.stateType ? ` • ${item.stateType}` : ''}`, 'state-flow', program.name, item.location, `Data: ${item.dataAccess.join(', ') || 'none'}\nLamports: ${item.lamportAccess.join(', ') || 'none'}\nField reads: ${item.fieldReads.join(', ') || 'none'}\nField writes: ${item.fieldWrites.join(', ') || 'none'}\nAccess evidence: ${item.accessSiteIds.length} site(s)${item.accessComplete ? '' : ' (incomplete)'}\nSerialization: ${item.serialization.join(', ') || 'unknown'}`));
    if (element.kind === 'instruction-asset-flows') return assetFlowItems(program, element.itemId);
    if (element.kind === 'instruction-state-accesses') return program.instructions.find(item => (item.id ?? item.name) === element.itemId)?.reachableSurface?.stateAccesses?.map(access => sourceOrText(`${access.operation} • ${access.accountName ?? access.accountExpression}${access.fieldPath ? `.${access.fieldPath}` : ''}${access.resolved ? '' : ' • unresolved account'}`, 'state-access', program.name, access.location, `${access.functionPath.join(' → ')}\nAlias path: ${access.aliasPath.join(' → ')}${access.api ? `\nAPI: ${access.api}` : ''}`)) ?? [];
...
function instructionChildren(program: ProgramUnit, instruction: InstructionInfo): Item[] { const surface = instruction.reachableSurface; const dossier = program.instructionDossiers?.find(item => item.instructionId === (instruction.id ?? instruction.name)); const flowIds = program.stateFlows?.filter(item => item.instructionId === (instruction.id ?? instruction.name)).map(item => item.id); const crossFunctions = new Set(surface?.crossPackageSurfaces?.flatMap(item => item.functions) ?? []); const localFunctions = surface?.functions.filter(item => !crossFunctions.has(item)); return [
  ...
  instructionSection(`Token / Asset Flows (${dossier?.assetFlows.length ?? 0})`, 'instruction-asset-flows', program.name, instruction.id ?? instruction.name),
  instructionSection(`Field / Lifecycle Accesses (${surface?.stateAccesses?.length ?? 0})`, 'instruction-state-accesses', program.name, instruction.id ?? instruction.name),
  ...
  new Item(`ABI • ${instruction.arguments?.map(argument => `${argument.name}: ${argument.type ?? 'unknown'}`).join(', ') || 'no arguments'} • returns ${instruction.returns ?? 'unknown'}`, vscode.TreeItemCollapsibleState.None, 'instruction-abi', program.name),
  new Item(instruction.remainingAccounts ? `Remaining accounts • ${instruction.remainingAccounts.onChainType ?? 'raw AccountMeta'} • bound ${instruction.remainingAccounts.onChainMax ?? 'unbounded'} • preserve input order` : 'Remaining accounts • none', vscode.TreeItemCollapsibleState.None, 'instruction-remaining-accounts', program.name),
...
function assetFlowItems(program: ProgramUnit, instructionId: string | undefined): Item[] {
  const dossier = program.instructionDossiers?.find(item => item.instructionId === instructionId);
  return (dossier?.assetFlows ?? []).map(flow => sourceOrText(`${flow.operation ?? 'token operation'} • ${flow.tokenProgram ?? 'unknown program'}${flow.pdaSigned ? ' • PDA signed' : ''}${flow.complete ? '' : ' • incomplete'}`, 'asset-flow', program.name, flow.location,
    [`Source: ${roleLabel(flow.source)}`, `Destination: ${roleLabel(flow.destination)}`, `Mint: ${roleLabel(flow.mint)}`, `Authority: ${roleLabel(flow.authority)}${flow.authorityType && flow.authorityType !== 'unresolved' ? ` (${flow.authorityType})` : ''}`,
     `Token program: ${flow.tokenProgram ?? 'unknown'}`, `Amount: ${flow.amount ?? 'unresolved'}`, `Decimals: ${flow.decimals ?? 'n/a'}`, `Signer PDAs: ${flow.signerPdaIds.join(', ') || 'none'}`,
     ...(flow.unresolvedReasons.length ? ['Unresolved:', ...flow.unresolvedReasons.map(reason => `- ${reason}`)] : [])].join('\n')));
}
function roleLabel(role: { accountName?: string; expression: string; resolved: boolean } | undefined): string { if (!role) return 'n/a'; return role.resolved && role.accountName ? role.accountName : `unresolved (${role.expression})`; }
