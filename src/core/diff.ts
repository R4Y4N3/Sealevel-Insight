import { ProgramUnit, TokenAssetFlow, WorkspaceReport } from '../model/report';
...
export function diffReports(before: WorkspaceReport, after: WorkspaceReport): ReportDiff {
  ...
  compareSection(changes, 'compilationProfile', new Map([['workspace', before.compilationProfile ?? {}]]), new Map([['workspace', after.compilationProfile ?? {}]]), ['target', 'mode', 'debugAssertions', 'cfgOptions', 'cfgKnowledge']);
  ...
  compareSection(changes, 'assetFlows',
...
function callGraphShape(program: ProgramUnit): Record<string, unknown> { return { calls: (program.callGraph?.calls ?? []).map(call => ({ id: call.id, status: call.status, target: call.target, candidates: call.candidateTargets, dispatchKind: call.dispatchKind, indirect: call.indirect, receiverType: call.receiverType, transforms: call.resolutionTransforms })).sort((a, b) => a.id.localeCompare(b.id)), cycles: (program.callGraph?.cycles ?? []).map(cycle => ({ id: cycle.id, kind: cycle.kind, functions: cycle.functions, callIds: cycle.callIds })).sort((a, b) => a.id.localeCompare(b.id)) }; }
/** Stable identity for a flow: program + instruction + the CPI site it came from. */
function assetFlowIdentity(program: ProgramUnit, flow: TokenAssetFlow): string { return `${program.packageId ?? program.name}:asset-flow:${flow.instructionId}:${flow.cpiId ?? `${flow.location.uri}:${flow.location.startLine}:${flow.location.startColumn}`}`; }
/** Comparable semantic shape; array-valued roles are order-insensitive. */
function assetFlowShape(flow: TokenAssetFlow): Record<string, unknown> { return { operation: flow.operation, tokenProgram: flow.tokenProgram, source: flow.source, destination: flow.destination, mint: flow.mint, authority: flow.authority, authorityType: flow.authorityType, delegate: flow.delegate, newAuthority: flow.newAuthority, amount: flow.amount, decimals: flow.decimals, pdaSigned: flow.pdaSigned, signerPdaIds: [...(flow.signerPdaIds ?? [])].sort(), complete: flow.complete }; }
...
function stable(value: unknown): string { return JSON.stringify(stableValue(value)); }
function stableValue(value: unknown): unknown { if (Array.isArray(value)) return value.map(stableValue); if (!value || typeof value !== 'object') return value; return Object.fromEntries(Object.entries(value as Record<string, unknown>).sort(([a], [b]) => a.localeCompare(b)).map(([key, child]) => [key, stableValue(child)])); }
function numericDelta(before: WorkspaceReport['summary'], after: WorkspaceReport['summary']): Record<string, number> { const result: Record<string, number> = {}; for (const key of Object.keys(after) as Array<keyof typeof after>) result[key] = (after[key] ?? 0) - (before[key] ?? 0); return result; }
