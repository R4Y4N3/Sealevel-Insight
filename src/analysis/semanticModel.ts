...
function extractStates(root: RustNode, uri: string, program: ProgramUnit): void {
  ...
  for (const struct of descendants(root, 'struct_item')) {
    ...
    const isState = /#\[(?:account|zero_copy)\b|\b(?:BorshSerialize|AnchorSerialize|Pod|Zeroable|Pack|ShankAccount|ShankType)\b|account!\s*\(/.test(`${attributes} ${struct.text}`);
    if (!isState) continue;
...
function runtimeKind(api: string): string | undefined { if (/invoke_signed|new_with_signer/.test(api)) return 'signed-cpi'; if (/(^|::)invoke$|CpiContext::new$/.test(api)) return 'cpi'; if (/set_return_data|get_return_data/.test(api)) return 'return-data'; if (/remaining_compute_units|sol_remaining_compute_units/.test(api)) return 'compute-units'; if (/keccak|sha256|hashv?/.test(api)) return 'hashing'; if (/curve|is_on_curve|alt_bn128|secp256/.test(api)) return 'curve-check'; if (/memcpy|memcmp|memset|memmove/.test(api)) return 'memory'; return undefined; }
