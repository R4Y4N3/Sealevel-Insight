...
export function applyReviewComplexity(program: ProgramUnit): ReviewHotspot[] {
  ...
  for (const fn of program.functions) {
    ...
    if (calls.some(call => call.status !== 'resolved')) reasons.push(`+${calls.filter(call => call.status !== 'resolved').length * REVIEW_WEIGHTS.unresolvedCalls} unresolved call surface`);
...
export function instructionComplexity(program: ProgramUnit, instruction: InstructionInfo): ReviewComplexity {
  ...
  const values: Array<[keyof typeof REVIEW_WEIGHTS, string, number]> = [
    ...
    ['pdas', 'PDAs', surface?.pdas.length ?? 0], ['remainingAccounts', 'remaining_accounts use', program.securitySurface.remainingAccounts ? 1 : 0],
    ['unsafe', 'unsafe surface', (surface?.unsafeFunctions?.length ?? 0) + (surface?.unsafeBlocks ?? 0)], ['manualSerialization', 'manual serialization', program.securitySurface.manualSerialization],
    ['realloc', 'reallocation', program.securitySurface.reallocOperations], ['unresolvedCalls', 'unresolved calls', surface?.unresolvedCalls?.length ?? 0], ['ambiguousCalls', 'ambiguous calls', surface?.ambiguousCalls?.length ?? 0]
  ];
