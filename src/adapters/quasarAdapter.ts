...
export function enrichQuasarSemantics(root: RustNode, uri: string): { instructions: InstructionInfo[]; accounts: AccountInfo[] } {
  ...
  for (const struct of descendants(root, 'struct_item')) {
    ...
    for (const fieldNode of descendants(struct, 'field_declaration')) {
      ...
      accounts.push({ id: `account:${uri}:quasar:${contextType}:${fieldNode.startPosition.row + 1}:${fieldNode.startPosition.column}:${name}`, name, type, wrapperType, stateType, contextType, signer: wrapperType === 'Signer' || constraints.some(item => item.kind === 'signer'), writable, executable: programWrapper, unchecked: wrapperType === 'UncheckedAccount', raw: wrapperType === 'AccountView', optional, addressExpectation: constraints.find(item => item.kind === 'address')?.expression ?? (programWrapper || wrapperType === 'Sysvar' ? stateType : undefined), ownerExpectation: wrapperType === 'SystemAccount' ? 'SystemProgram' : undefined, ownerValidated: ['Account', 'InterfaceAccount', 'SystemAccount'].includes(wrapperType ?? ''), addressValidated: programWrapper || wrapperType === 'Sysvar' || constraints.some(item => item.kind === 'address'), constraints, relations, lifecycle: [...new Set(lifecycle)], location: loc(uri, fieldNode), confidence: 0.93, evidence: [{ description: 'Quasar derive(Accounts) field and account constraints', location: loc(uri, fieldNode) }] });
    }
  }
