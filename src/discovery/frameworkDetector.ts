...
export function detectFramework(source: string, uri: string): FrameworkEvidence[] {
  ...
  const add = (framework: string, confidence: number, matches: string[]) => {
    ...
    if (found.length) evidence.push({ framework, confidence, evidence: found, location: { uri, startLine: 1, startColumn: 0, endLine: 1, endColumn: 0 } });
  };
  ...
  if (!evidence.length && /(invoke|AccountInfo|process_instruction|entrypoint)/.test(source)) {
    evidence.push({ framework: 'native-or-custom', confidence: 0.55, evidence: ['generic Solana entrypoint/account signal'], location: { uri, startLine: 1, startColumn: 0, endLine: 1, endColumn: 0 } });
  }
  return evidence;
}
