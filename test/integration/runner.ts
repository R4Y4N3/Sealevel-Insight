...
export async function run(): Promise<void> {
  ...
  if (report.schemaVersion !== extension.packageJSON?.version || !Array.isArray(report.files) || !report.programs[0]?.instructions.length) throw new Error(`Analysis returned an incomplete report: ${JSON.stringify(report)}`);
